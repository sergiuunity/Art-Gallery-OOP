...
void Gallery::append(Painting p, bool ignoreStacks)
{
	...
	{
		if(ignoreStacks == false)
			undo_stack.push(Action(1, p));
		gallery.push_back(p);
	}
	else
	{
...
Painting Gallery::popBack()
{
	...
	this->remove_at_index(this->size() - 1, false);
	return temp;
}

Painting Gallery::remove_at_index(int index, bool ignoreStacks)
{
...
void Gallery::load_from_file(string path)
{
	...
	while (input_file.good())
	{
		...
		this->append(Painting(id, name, author, stoi(estimated_price), stoi(width), stoi(height)), true);
	}
	input_file.close();
}
...
void Gallery::undo()
{
	...
	if (undo_stack.empty() == false)
	{
		...
		if (last_action.getType() == -1)
			this->append(last_action.getParameter(), true);
		else if (last_action.getType() == 1)
			this->remove(last_action.getParameter());
		undo_stack.pop();
	}
	else
		throw(-3);
